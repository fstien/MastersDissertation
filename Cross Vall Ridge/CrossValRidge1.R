...
for(d in 1:1) { 
  ...
  out = foreach(i = 1:k, .packages = c("DEoptimR"), .combine = rbind) %dopar% {
    try({
      ...
      set.seed(1234)
      theta <- JDEoptim(rep(-1, nCat), rep(1, nCat), fn=meanUOptim, constr=consRidge, maxiter=50)#, trace = TRUE, triter = 1)
      return(theta$par)
    })
  }
  stopCluster(cl)
  ...
  ridgeFold[ridgeFold$d == d,]$SR <- SR

  write.csv(file=paste("ridgeFold", d, ".csv", sep=""), x=ridgeFold, row.names=FALSE)
}

print("All done writing ridgeFold.")
write.csv(file="ridgeFold.csv", x=ridgeFold, row.names=FALSE)
